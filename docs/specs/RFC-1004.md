# RFC-1004: Agent Kernel and Sandboxed Effect Executor Model

**Status**: Draft
**Authors**: chenxm
**Created**: 2026-03-01
**Last Updated**: 2026-03-07
**Refinement**: Updated RFC number in title and internal references
**Depends on**: [RFC-1003](RFC-1003.md)
**Supersedes**: ---
...
This RFC defines the architectural abstraction and design principles for the Agent Kernel and its interaction with sandboxed effect executors for tool execution in a multi-user AI system.
...
- Sandboxed Effect Executor model for external effect execution
- Deterministic execution boundary between kernel and effect executors
...
These operations are non-deterministic and may be influenced by user prompts. Direct execution within the kernel violates determinism, security, and isolation requirements.

Therefore:

> The Agent Kernel MUST remain pure and deterministic.
> All external effects MUST be delegated to sandboxed effect executors.
...
3. **External Isolation**: Sandboxed effect executors enforce security boundaries outside the kernel.
...
- Capability validation via RFC-1003
- Effect graph construction
- Effect executor orchestration
...
### 5.2 Effect Executor Model

An **Effect Executor** is an isolated execution unit created when the Kernel requires an external effect.
...
Effect executors are ephemeral and destroyed after execution completes.

**Terminology note**: The Effect Executor in this RFC (sandboxed tool/effect runner) is distinct from the **Subagent** in [RFC-1005](RFC-1005.md) and [RFC-1006](RFC-1006.md), which denotes a delegated *cognitive agent* (e.g. Tacitus, BrowserUse) that extends the orchestrator's reasoning.
...
2. Capability registry validates request per RFC-1003
3. Effect executor is spawned in sandbox
...
2. **Memory Isolation**: Effect executors cannot access kernel memory.
3. **User Isolation**: Effect executors cannot access other users' state.
...
- **RFC-1003**: Depends on Capability Registry for capability validation protocol
- **RFC-1003**: Implementation architecture for OpenSandbox-based effect executor execution
- **RFC-1005**, **RFC-1006**: Define **Subagent** (delegated cognitive agent); distinct from this RFC's Effect Executor (sandboxed tool runner)

---

## 14. Open Questions

- Should effect executors support persistent state across invocations for long-running operations?
...
This RFC establishes the architectural foundation for secure, deterministic agent operation with sandboxed tool execution. By separating the pure kernel from effect executors, the system achieves:
...
> The Agent Kernel MUST remain pure and deterministic. All external effects MUST be delegated to sandboxed effect executors.
