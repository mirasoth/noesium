...
| AskuraAgent | RFC-1002 §5.2 | Conversation agent with built-in session management and HITL |
...
| Cognitive Loop | RFC-0007 §7 | Continuous runtime loop that drives autonomous execution by repeatedly selecting goals, generating reasoning, performing actions, and updating memory |
...
| Event Queue | RFC-0009 §10 | Internal queue maintaining pending events and processing state for sequential deterministic execution |
| Event Store | RFC-0001 §5.3 | Append-only event log per agent |
| Event System | RFC-0009 §1 | Reactive layer enabling the autonomous agent runtime to detect and respond to environment changes through unified event model and trigger rules |
...
| Goal | RFC-0008 §5 | Objective the agent should attempt to complete, with id, description, priority, status, and lifecycle |
| Goal Engine | RFC-0007 §6 | Minimal component managing goal creation, prioritization, scheduling, and lifecycle without performing reasoning |
| Goal Queue | RFC-0008 §8 | Priority queue of active goals maintained by Goal Engine with highest_priority_first scheduling |
...
| Trigger Rules | RFC-0009 §8 | Rules that convert events into goals by defining conditions and goal templates |
...
| Application Layer | RFC-1007 §5.2 | noeagent; complete agent application depending on all lower layers |
