...
**Depends on**: [RFC-1001](RFC-1001.md), [RFC-1003](RFC-1003.md)
**Supersedes**: ---
...
* Tool capability registration via [RFC-1003](RFC-1003.md) capability registry
* Skill management architecture (tool composition)
* Security and sandboxing principles
* Integration with the agent kernel

### 2.2 Non-Goals

* Concrete Python interfaces or module layout (see [RFC-2103](RFC-2103.md))
...
3. **Capability First**: Every tool MUST register itself as a capability ([RFC-1003](RFC-1003.md)) with determinism class, side-effect class, and input/output schema. Agents discover tools through the capability registry.
...
| `determinism_class` | `deterministic` or `nondeterministic` ([RFC-1003](RFC-1003.md)) |
| `side_effect_class` | `pure`, `idempotent`, or `external` ([RFC-1003](RFC-1003.md)) |
| `execute(input) -> output` | The callable that performs the action |

Every AtomicTool is also a Capability in the [RFC-1003](RFC-1003.md) sense.
...
Tools register as capabilities via the existing capability registry ([RFC-1003](RFC-1003.md)):
...
* **[RFC-1001](RFC-1001.md)**: Tool events follow the envelope specification.
* **[RFC-1003](RFC-1003.md)**: Tool capabilities are registered in the capability registry.
* **[RFC-1003](RFC-1003.md)**: Each tool registers as a capability with determinism/side-effect classification.
* **[RFC-2001](RFC-2001.md)**: `ToolExecutor` uses `EventStore` and `EnvelopeBridge` for event emission.
* **[RFC-2002](RFC-2002.md)**: Agent graphs use tool nodes that route through `ToolExecutor`.
