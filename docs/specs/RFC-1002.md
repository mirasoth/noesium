...
2. **SearchAgent** (removed; superseded by `wizsearch` toolkit) — A task-oriented agent using `BaseGraphicAgent` with linear graph flow and optional conditional branches
