...
2. **SearchAgent** (removed; superseded by `web_search` toolkit) — A task-oriented agent using `BaseGraphicAgent` with linear graph flow and optional conditional branches
3. **TacitusAgent** — A research agent using `BaseResearcher` with iterative loops, parallel `Send` dispatching, and structured LLM output
