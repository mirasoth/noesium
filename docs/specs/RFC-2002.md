# RFC-2002: LangGraph-Based Agent Implementation Design

**Status**: Draft
**Authors**: Noesium Team
**Created**: 2026-03-01
**Last Updated**: 2026-03-07
**Depends on**: [RFC-1003](RFC-1003.md), [RFC-2001](RFC-2001.md)
**Supersedes**: ---
...
This RFC defines the implementation design for LangGraph-based agents in Noesium. It specifies how the deterministic execution model and capability registry ([RFC-1003](RFC-1003.md)) are realized through concrete LangGraph patterns. The RFC defines three agent archetypes — Conversation, Research, and Task — along with their state models, graph construction patterns, node execution contracts, human-in-the-loop workflows, checkpointing strategies, and lifecycle management. This RFC builds on the core infrastructure defined in [RFC-2001](RFC-2001.md) and provides the implementation contract for all Noesium agents.
...
* Integration with [RFC-2001](RFC-2001.md) kernel executor and event system

### 2.2 Non-Goals

This RFC does **not** define:

* Core infrastructure (event system, projection layer, capability registry) — see [RFC-2001](RFC-2001.md)
...
Noesium agents are implemented as LangGraph `StateGraph` workflows. The current codebase contains two distinct agent patterns:

1. **AskuraAgent** — A conversation agent with built-in session management, HITL interrupts, and in-memory checkpointing
2. **TacitusAgent** — A research agent with iterative loops, parallel `Send` dispatching, and structured LLM output
...
2. Define node execution contracts aligned with determinism goals
3. Formalize state management to enable event-sourced projection ([RFC-2001](RFC-2001.md))
...
| Property | Value |
|----------|-------|
| Concrete agent | `AskuraAgent` |
| Base class | `BaseGraphicAgent` |
...
**Purpose**: Iterative information gathering with reflection and source management.

**Characteristics**:

| Property | Value |
|----------|-------|
| Concrete agent | `TacitusAgent` |
| Base class | `BaseGraphicAgent` |
| State model | `TypedDict` with `Annotated` reducers |
| Checkpointing | Optional |
| HITL | Not required |
| Parallelism | `Send` for concurrent web searches |
| Execution | Async `ainvoke()` or `astream()` |

**Reference graph topology** (TacitusAgent):
...
For integration with the kernel executor ([RFC-2001](RFC-2001.md)), nodes SHOULD be annotated:
...
```python
class MyConversationAgent(BaseGraphicAgent):
    def __init__(self, config: MyConfig):
        super().__init__(llm_provider=config.llm_provider)
        self._session_states: Dict[str, Any] = {}  # Built-in session management
...
```python
class MyResearchAgent(BaseGraphicAgent):
    def __init__(self, config: MyConfig):
        super().__init__(llm_provider=config.llm_provider)
        self.config = config
        self.graph = self._build_graph()
    ...
    async def research(self, topic: str, context: Dict = None) -> ResearchOutput:
...
Existing agents can integrate with the [RFC-2001](RFC-2001.md) kernel executor incrementally:
...
* **[RFC-1003](RFC-1003.md)**: `declare_capability()` implements capability registration. Agent capabilities are discoverable via the projection-based registry.
* **[RFC-2001](RFC-2001.md)**: `KernelExecutor` wraps LangGraph execution with event emission and checkpointing. `BaseAgent` extensions provide event store, projection engine, and memory manager. Agent configuration uses `ExecutionMode` for kernel integration.

---

## 20. Open Questions

1. Should conversation agents enforce async execution to align with research agents?
