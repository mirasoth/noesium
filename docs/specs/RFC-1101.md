...
This RFC defines the abstract architecture for memory management in the Noesium event-driven framework. It unifies the existing event-sourced memory ([RFC-1002](RFC-1002.md)) and file-based MemU system into a single Memory Provider model with three tiers: Working Memory, Persistent Memory, and Indexed Memory. It also declares the abstract contract for a future graph-based memory provider.
...
* Integration with the event system ([RFC-1001](RFC-1001.md))

### 2.2 Non-Goals

This RFC does **not** define:

* Concrete Python interfaces or module layout (see [RFC-2102](RFC-2102.md))
...
1. **Event-driven stack** ([RFC-1002](RFC-1002.md), [RFC-2001](RFC-2001.md)): `EphemeralMemory`, `DurableMemory`, `SemanticMemory`, and `MemoryManager` -- built on `EventStore` and projections.
...
| Examples | Current conversation context, intermediate reasoning, scratchpad, CognitiveContext |

Working Memory is always in-process and dict-backed. It requires no IO.

> **Implementation Note**: [CognitiveContext (RFC-1010)](RFC-1010.md) is a specialized Working Memory content type providing a 3-field model (goal, findings, scratchpad) for agent state continuity across TUI turns. It operates independently of event-sourced infrastructure.
...
All operations MUST be async. Write operations on Persistent providers MAY emit events for audit/observability but this is OPTIONAL in the current simplified implementation.

> **Implementation Note**: The current implementation prioritizes simplicity over full event-sourcing. Working Memory (including CognitiveContext) and MemU-backed Persistent Memory operate without mandatory event emission. EventStore integration is available as an optional advanced feature.

### 6.2 Provider Registry

Providers register themselves with the memory subsystem. Registration follows the same event-driven pattern as capabilities (RFC-1003):
...
All memory lifecycle events follow the envelope format (RFC-1001):
...
* **[RFC-0001](RFC-0001.md)**: Memory is part of the event-sourced agent architecture. All persistent writes produce events.
* **[RFC-1001](RFC-1001.md)**: Memory events follow the envelope specification.
* **[RFC-1002](RFC-1002.md)**: This RFC supersedes RFC-1002's memory hierarchy with a more extensible provider-based model. The three tiers (ephemeral, durable, semantic) map to (Working, Persistent, Indexed).
* **[RFC-2001](RFC-2001.md)**: The existing `MemoryManager`, `DurableMemory`, etc. become the event-sourced provider implementation under this architecture.
* **[RFC-1003](RFC-1003.md)**: Memory providers register like capabilities, enabling discovery.
