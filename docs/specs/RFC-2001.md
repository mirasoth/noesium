...
The Noesium framework currently implements a functional agent system built on LangGraph with direct tool invocation, mutable state management, and in-process event dispatching via bubus. While operational, the current implementation diverges significantly from the event-sourced, deterministic architecture described in [RFC-0001](RFC-0001.md), [RFC-1001](RFC-1001.md), [RFC-1002](RFC-1002.md), and [RFC-1003](RFC-1003.md).
...
* `core/kernel/` — Deterministic kernel execution layer
...
The canonical event envelope implements [RFC-1001](RFC-1001.md):
...
The kernel executor wraps LangGraph execution with deterministic execution constraints:
...
class KernelExecutor:
    """Deterministic execution wrapper over LangGraph."""
...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self.agent_ref = AgentRef(
            ...
            runtime_id=os.getenv("NOE_RUNTIME_ID", "local"),
...
Failures produce events per [RFC-1001, Section 14](RFC-1001.md#14-error-events):
...
* **[RFC-1001](RFC-1001.md)**: The `EventEnvelope` class directly implements the canonical envelope specification.
* **[RFC-1002](RFC-1002.md)**: The `BaseProjection`, `ProjectionEngine`, and memory hierarchy implement the formal projection and memory model.
* **[RFC-1002](RFC-1002.md)**: The `BaseProjection`, `ProjectionEngine`, and memory hierarchy implement the formal projection and memory model.
* **[RFC-1003](RFC-1003.md)**: The `CapabilityRegistry`, `DiscoveryService`, and `DeterministicResolver` implement the capability registry and discovery protocol.
