...
class ExtractionResult(BaseModel):
    ...
    is_partial: bool = Field(
        default=False, description="True if content was truncated before extraction"
    )
    source_url: str | None = Field(
        default=None, description="URL the content was extracted from"
    )
    content_stats: dict[str, Any] = Field(
        default_factory=dict, description="Content processing statistics"
    )
