...
from noesium.core.tracing import configure_opik, estimate_token_usage, get_token_tracker
...
class LLMClient(BaseLLMClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not LITELLM_AVAILABLE:
            raise ImportError(
                "LiteLLM package is not installed. Install it with: pip install 'noesium[litellm]'"
            )
        ...
        self.embed_model = embed_model or os.getenv(
            "LITELLM_EMBED_MODEL", "text-embedding-ada-002"
        )
    ...
    def _get_content_filter_provider(self, e: Exception) -> str:
        ...
        if "dashscope" in error_str:
            return "Dashscope/Alibaba Cloud"
        # Try to extract model prefix (e.g., "claude-" -> "Anthropic")
            ...
            if stream:
                ...
                try:
                    if hasattr(response, "usage") and response.usage:
                        ...
                        logger.debug(
                            f"Token usage for completion: {usage.total_tokens} tokens"
                        )
                    else:
        ...
        **kwargs,
        ...
        for i in range(attempts):
            try:
                ...
                try:
                    prompt_text = "\n".join(
                        [msg.get("content", "") for msg in structured_messages]
                    )
