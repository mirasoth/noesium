...
class BaseWatchdog(BaseModel, Generic[TEventProcessor]):
    ...
    event_processor: Any = (
        Field()
    )  # Use Any to avoid Pydantic validation issues with generic types
            ...
            async def unique_handler(event):
                ...
                try:
                    ...
                    result_summary = (
                        ""
                        if result is None
                        else f" ➡️ {color_text(f'<{type(result).__name__}>', 'magenta')}"
                    )
                    event_processor.logger.debug(
                    ...
                    try:
                        await watchdog_instance._handle_handler_error(
                            e, event, actual_handler
                        )
    ...
    async def _handle_handler_error(
        self, error: Exception, event: BaseEvent[Any], handler
    ) -> None:
    ...
    def attach_to_processor(self) -> None:
        ...
        if self.LISTENS_TO:
            event_class_map = {
                event_class.__name__: event_class for event_class in self.LISTENS_TO
            }
            self.logger.debug(
            ...
            try:
                ...
                if hasattr(self.event_bus, "_event_types"):
                    event_class_map = {
                        cls.__name__: cls for cls in self.event_bus._event_types
                    }
                elif hasattr(self.event_bus, "event_registry"):
                    event_class_map = {
                        name: cls for name, cls in self.event_bus.event_registry.items()
                    }
                else:
                    ...
                    for event_name, handlers in self.event_bus.handlers.items():
                        if handlers and hasattr(handlers[0], "__annotations__"):
                            ...
                            if "event" in annotations:
                                ...
                                if isinstance(event_class, type) and issubclass(
                ...
                self.logger.warning(
                    f"[{self.__class__.__name__}] Failed to discover event classes: {e}"
                )
        ...
        for method_name in handler_methods:
            ...
            if event_class:
                ...
                self.attach_handler_to_processor(
                    self.event_processor, event_class, handler
                )
                registered_events.add(event_class)

                self.logger.debug(
                    f"[{self.__class__.__name__}] Registered handler {method_name} for {event_name}"
                )
            else:
        ...
        self.logger.info(
            f"[{self.__class__.__name__}] Successfully registered {len(registered_events)} event handlers"
        )
