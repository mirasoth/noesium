...
if TYPE_CHECKING:
    from noesium.agents.browser_use.browser.session import BrowserSession, CDPSession, Target
...
class SessionManager:
    ...
    async def _handle_target_attached(self, event: AttachedToTargetEvent) -> None:
        ...
        if target_id not in self._targets:
            from noesium.agents.browser_use.browser.session import Target
        ...
        from noesium.agents.browser_use.browser.session import CDPSession
    ...
    async def _handle_target_detached(self, event: DetachedFromTargetEvent) -> None:
        ...
        async with self._lock:
            ...
            if target_id in self._target_sessions:
                ...
                if remaining_sessions == 0:
                    self.logger.debug(
                        f"[SessionManager] No sessions remain for target {target_id[:8]}..., removing target"
                    )
        ...
        if target_fully_removed:
            if target_type in ("page", "tab"):
                from noesium.agents.browser_use.browser.events import TabClosedEvent
    ...
    async def _recover_agent_focus(self, crashed_target_id: TargetID) -> None:
        ...
        try:
            ...
            async with self._recovery_lock:
                ...
                if self.browser_session._cdp_client_root is None:
                    self.logger.debug(
                        "[SessionManager] Skipping focus recovery - browser shutting down (no CDP client)"
                    )
                    return
                ...
                if (
                    ...
                    and self.browser_session.agent_focus_target_id != crashed_target_id
                ):
                    self.logger.debug(
            ...
            if page_targets:
                ...
                from noesium.agents.browser_use.browser.events import TabCreatedEvent
            ...
            if new_session:
                ...
                from noesium.agents.browser_use.browser.events import AgentFocusChangedEvent
            ...
            for _ in range(20):
                ...
                if fallback_session:
                    ...
                    from noesium.agents.browser_use.browser.events import AgentFocusChangedEvent, TabCreatedEvent
