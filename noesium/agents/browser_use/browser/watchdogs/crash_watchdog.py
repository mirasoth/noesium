...
class CrashWatchdog(BaseWatchdog):
    ...
    _sessions_with_listeners: set[str] = PrivateAttr(
        default_factory=set
    )  # Track sessions that already have event listeners
    ...
    async def attach_to_target(self, target_id: TargetID) -> None:
        ...
        try:
            ...
            if cdp_session.session_id in self._sessions_with_listeners:
                self.logger.debug(
                    f"[CrashWatchdog] Event listeners already exist for session: {cdp_session.session_id}"
                )
                return
    ...
    async def _start_monitoring(self) -> None:
        ...
        assert (
            self.browser_session.cdp_client is not None
    ...
    async def _check_browser_health(self) -> None:
        ...
        try:
            try:
                self.logger.debug(
                    f"[CrashWatchdog] Checking browser health for target {self.browser_session.agent_focus}"
                )
            ...
            self.logger.debug(
                f"[CrashWatchdog] Attempting to run simple JS test expression in session {cdp_session} 1+1"
            )
            ...
            self.logger.debug(
                f"[CrashWatchdog] Browser health check passed for target {self.browser_session.agent_focus}"
            )
        except Exception as e:
            self.logger.error(
        ...
        if self.browser_session._local_browser_watchdog and (
            proc := self.browser_session._local_browser_watchdog._subprocess
        ):
            try:
