...
class CrashWatchdog(BaseWatchdog):
    ...
    async def _on_target_crash_cdp(self, target_id: TargetID) -> None:
        ...
        if is_agent_focus:
            self.logger.error(
                f"[CrashWatchdog] 💥 Agent focus tab crashed: {target.url} (SessionManager will auto-recover)"
            )
    ...
    async def _check_browser_health(self) -> None:
        ...
        try:
            self.logger.debug(
                f"[CrashWatchdog] Checking browser health for target {self.browser_session.agent_focus_target_id}"
            )
