...
Replace all imports from `browser_use.` to `noesium.subagents.bu.`:

```bash
find noesium/agents/browser_use -name "*.py" -exec sed -i '' 's/from browser_use\./from noesium.subagents.bu./g' {} \;
find noesium/agents/browser_use -name "*.py" -exec sed -i '' 's/import bu/from noesium.subagents import bu/g' {} \;
```
...
def __getattr__(name: str):
    if name in ("NoesiumAgentAdapter", "create_agent_adapter"):
        from noesium.subagents.bu.adapters import agent_adapter
...
Test basic imports:
```bash
python -c "from noesium.subagents.bu import BrowserProfile, BrowserSession"
python -c "from noesium.subagents.bu.adapters import NoesiumLLMAdapter, NoesiumAgentAdapter"
```
...
6. Fixed import paths from `browser_use.` to `noesium.subagents.bu.`
